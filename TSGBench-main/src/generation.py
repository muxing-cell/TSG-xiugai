...
from model.my_timevae_model import MyTimeVAE
from model.my_timevqvae_model import MyTimeVQVAE
from model.my_timegan_model import MyTimeGAN
from model.my_diffusionts_model import MyDiffusionTS
from model.my_fourierflow_model import MyFourierFlow
...
def generate_data(cfg,data):
    ...
    if model_name == 'TimeVAE':
        model = MyTimeVAE(cfg, train_data[0].shape)
    elif model_name == 'TimeVQVAE':
        model = MyTimeVQVAE(cfg, train_data[0].shape)
    elif model_name == 'TimeGAN':
        model = MyTimeGAN(cfg, train_data[0].shape)
    elif model_name == 'DiffusionTS':
        model = MyDiffusionTS(cfg, train_data[0].shape)
    elif model_name == 'FourierFlow':
        model = MyFourierFlow(cfg, train_data[0].shape)
    else:
